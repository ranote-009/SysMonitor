...
std::atomic<bool> Server::shouldRun_{true};
 
// Signal handler for SIGINT and SIGTERM
void Server::signalHandler(int signal) {
    if ((signal == SIGINT || signal == SIGTERM) && shouldRun_) {
        std::cout << "\n\nReceived termination signal. Server Shutting down.\n\n" << std::endl;
        shouldRun_ = false;
...
void Server::HandleClient(ssl::stream<tcp::socket>&& stream) {
        try {
            ...
            if (VerifyConnectionKey(ws, connectionKey_)) {
                ...
                //HandleDisconnectMessage(ws, connectionKey_);
            }
            else{
            beast::error_code ec;
            ...
            while (1) {
                ...
                try {
                    ...
                    auto event_node = received_tree.find("event");
                    if (event_node != received_tree.not_found()) {
                        // The "event" field is present, handle accordingly
                        string event = event_node->second.get_value<string>();
                        //string event = "connected";
                        //event = received_tree.get<string>("event");
                        ...
                        if (event == "disconnect") {
                            ...
                            //     //cout << stream.lowest_layer().remote_endpoint() << " is disconnected." << endl;
                            // }
                            cout<<"\n   \n  client is disconnected:      "<< macaddress<< endl;
                ...
                // Warning analysis       
                   ...
                   int ramUsagePercentage = (stoi(ramUsage)*100) / stoi(ramUsage.substr(15,4));
                    string ramUsageP=to_string(ramUsagePercentage);
                    ...
                    try {
                        ...
                        db.StoreSystemInfo(macaddress, hostname, cpuUsage, ramUsageP,hdd_utilization);
                    } catch (const std::exception& e) {
